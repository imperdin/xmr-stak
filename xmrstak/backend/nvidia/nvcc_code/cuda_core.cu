		...
		uint32_t startNonce, uint32_t * __restrict__ d_input )
{
	...
	uint32_t tweak1_2[2];
	if (ALGO == cryptonight_monero || ALGO == cryptonight_aeon || ALGO == cryptonight_ipbc || ALGO == cryptonight_stellite || ALGO == cryptonight_masari || ALGO == cryptonight_saber)
	{
	...
	if(ALGO == cryptonight_heavy || ALGO == cryptonight_haven || ALGO == cryptonight_saber || ALGO == cryptonight_italo)
	{
		if(partidx != 0)
		{
	...
	for ( i = start; i < end; ++i )
	{
		...
		for ( int x = 0; x < 2; ++x )
		{
			...
			else
			{
				...
				d[x] = a ^
					t_fn0(__byte_perm(x_0, 0, 0x4440)) ^
					t_fn1(__byte_perm(x_1, 0, 0x4441)) ^
					t_fn2(__byte_perm(x_2, 0, 0x4442)) ^
					t_fn3(__byte_perm(x_3, 0, 0x4443));
			}
			...
			if(ALGO == cryptonight_monero || ALGO == cryptonight_aeon || ALGO == cryptonight_ipbc || ALGO == cryptonight_stellite || ALGO == cryptonight_masari || ALGO == cryptonight_saber)
			{
				...
				if(ALGO == cryptonight_monero || ALGO == cryptonight_aeon || ALGO == cryptonight_ipbc || ALGO == cryptonight_masari || ALGO == cryptonight_saber)
				{
            ...
            *( (uint64_t *) t2 ) += sub2 ? ( *( (uint64_t *) t1 ) * *( (uint64_t*) zz ) ) : __umul64hi( *( (uint64_t *) t1 ), *( (uint64_t*) zz ) );
			...
			if(ALGO == cryptonight_monero || ALGO == cryptonight_aeon || ALGO == cryptonight_ipbc || ALGO == cryptonight_stellite || ALGO == cryptonight_masari || ALGO == cryptonight_saber)
			{
				...
				if (ALGO == cryptonight_ipbc || ALGO == cryptonight_saber)
				{
			...
			idx0 = shuffle<4>(sPtr,sub, a, 0);
			if(ALGO == cryptonight_heavy || ALGO == cryptonight_saber)
			{
				idx0 = (( idx0 & MASK ) >> 3);
...
void cryptonight_core_gpu_hash(nvid_ctx* ctx, uint32_t nonce)
{
	...
	for ( int i = 0; i < partcountOneThree; i++ )
	{
		CUDA_CHECK_KERNEL(ctx->device_id, cryptonight_core_gpu_phase1<ITERATIONS,MEMORY><<< grid, block8 >>>( ctx->device_blocks*ctx->device_threads,
			bfactorOneThree, i,
			ctx->d_long_state,
			((ALGO == cryptonight_heavy || ALGO == cryptonight_haven || ALGO == cryptonight_saber || ALGO == cryptonight_italo) ? ctx->d_ctx_state2 : ctx->d_ctx_state),
			ctx->d_ctx_key1 ));
...
void cryptonight_core_cpu_hash(nvid_ctx* ctx, xmrstak_algo miner_algo, uint32_t startNonce)
{
	...
	else if(miner_algo == cryptonight_saber)
	{
	  cryptonight_core_gpu_hash<CRYPTONIGHT_HEAVY_ITER, CRYPTONIGHT_HEAVY_MASK, CRYPTONIGHT_HEAVY_MEMORY/4, cryptonight_saber>(ctx, startNonce);
	}
	else if(miner_algo == cryptonight_italo)
	{
	  cryptonight_core_gpu_hash<CRYPTONIGHT_HEAVY_ITER, CRYPTONIGHT_HEAVY_MASK, CRYPTONIGHT_HEAVY_MEMORY/4, cryptonight_italo>(ctx, startNonce);
	}
